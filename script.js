...
const playersTable = document.querySelector("#player-rows");
...
playerSearchInput.addEventListener("input", () => {
  ...
  for (let i = 0; i < data.length; i++) {
    if (data[i].name.toLowerCase().includes(query)) {
      matchedPlayers.push(data[i]);
    }
  }
  displayPlayers(matchedPlayers);
});

const teamDropdown = document.querySelector("#team-filter");
const teams = [];
for (let i = 0; i < data.length; i++) {
  if (teams.indexOf(data[i].team) === -1) {
...
const themeSwitcher = document.querySelector("#dark-mode-toggle");
